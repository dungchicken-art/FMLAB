...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded).permute(1, 2, 0) # (b, e, src_len)

        # Apply the post_embed_cnn layer and apply the `pack_padded_sequence` function to X
        X = self.post_embed_cnn(X).permute(2, 0, 1) # (src_len, b, e)
        ...
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2) # (src_len, b, h*2) -> (b, src_len, h*2)
        
        # Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        init_decoder_hidden = self.h_projection(torch.cat([*last_hidden], dim=1)) # (2, b, h) -> (b, 2*h), (b, 2*h) @ (2*h, h) -> (b, h)
        init_decoder_cell   = self.c_projection(torch.cat([*last_cell],   dim=1)) # (2, b, h) -> (b, 2*h), (b, 2*h) @ (2*h, h) -> (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Apply the attention projection layer to `enc_hiddens` to obtain `enc_hiddens_proj` with shape (b, src_len, h)
        ...
        # Use torch.stack to convert combined_outputs from a list length tgt_len (b, h) -> (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2) # (b, src_len, h) @ (b, h, 1) -> (b, src_len, 1)
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1) # (b, 1, src_len) @ (b, src_len, 2h) -> (b, 1, 2h)
